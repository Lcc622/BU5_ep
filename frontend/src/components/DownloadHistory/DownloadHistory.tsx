...
export function DownloadHistory() {
 ...
 <section className="rounded-[32px] border border-[#E8EAED] bg-white p-6 shadow-panel">
  ...
  <button
   ...
   onClick={() => refetch()}
   disabled={isFetching}
   className=" border border-[#E8EAED] px-4 py-2 text-sm font-semibold text-ink transition hover:border-ink disabled:opacity-60"
  >
   {isFetching ? '刷新中...' : '刷新列表'}
  </button>
  </div>

  <div className="mt-6 overflow-hidden border border-[#E8EAED]">
   ...
   {!isLoading && !isError
    ? files.map((file) => (
     ...
     <td className="px-4 py-4 text-right">
      ...
      download
      className="inline-flex border border-ink px-4 py-2 text-sm font-semibold text-ink transition hover:bg-ink hover:text-white"
      >
      下载
      </a>
     </td>
