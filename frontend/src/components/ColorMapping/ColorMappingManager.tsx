...
export function ColorMappingManager() {
 ...
 return (
 <section className="rounded-[32px] border border-[#E8EAED] bg-white p-6 shadow-panel">
  ...
  <div className=" bg-mist px-4 py-2 text-sm font-semibold text-ink">
  ...
  <div>
   ...
   <button
    ...
    className=" bg-[#091E40] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#0D2B52] disabled:opacity-60"
   >
    搜索
   </button>
   <button
    ...
    className=" border border-[#E8EAED] px-5 py-3 text-sm font-semibold text-steel transition hover:border-ink hover:text-ink"
   >
    清空
   </button>
   </form>

   <div className="mt-4 overflow-hidden border border-[#E8EAED]">
   <table className="min-w-full divide-y divide-slate-200">
    ...
    {!isLoading && !isError
     ? entries.map(([code, names]) => (
      ...
      <td className="px-4 py-4 text-sm text-[#8C8C8C]">{names.fr || '—'}</td>
      <td className="px-4 py-4 text-sm text-[#8C8C8C]">{names.de || '—'}</td>
      <td className="px-4 py-4 text-sm text-[#8C8C8C]">{names.it || '—'}</td>
      <td className="px-4 py-4 text-sm text-[#8C8C8C]">{names.es || '—'}</td>
  ...
  <div className=" bg-mist p-5">
   ...
   <input
    ...
    maxLength={2}
    className="w-full border border-white bg-white px-4 py-3 outline-none transition focus:border-signal"
   />
   {LANGS.map(({ key, label }) => (
    ...
    <input
     ...
     className="w-full border border-white bg-white px-4 py-2.5 text-sm outline-none transition focus:border-signal"
    />
    </div>
   ))}
   <button
    ...
    className="w-full bg-signal px-5 py-3 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-60"
   >
